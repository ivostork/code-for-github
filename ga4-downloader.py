...
try:
    ...
    while aktualni_mesic <= END_TOTAL:
        ...
        pripadny_konec = aktualni_mesic + relativedelta(months=1) - relativedelta(days=1)
        ...
        while True:
            ...
            request = RunReportRequest(
                ...
                date_ranges=[DateRange(start_date=start_str, date_range_name="", end_date=end_str)],
                limit=API_LIMIT,
                offset=offset
            )
            ...
            if je_to_prvni_zapis:
                print(f"  📥 [RESET] Zakládám novou optimalizovanou tabulku s partitioningem...")
            ...
            job_config = bigquery.LoadJobConfig(write_disposition=dispozice_zapisu)
            job = bq_client.load_table_from_dataframe(df, full_table_path, job_config=job_config)
            job.result()
            
            print(f"Uloženo. (V měsíci: {radku_v_mesici:,} | Celkem v cloudu: {celkem_odeslano_radku:,} řádků)")
